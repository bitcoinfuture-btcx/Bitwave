...
#if defined(NDEBUG)
# error "Bitwave cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x42da9c63febc73365e088c4755d6980bc4eef97fba23f7ba38c29007e7d399c9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitwave: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitwave Signed Message:\n";
...
{
    // Bitwave: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Bitwave: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Bitwave: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Bitwave: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitwave: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitwave: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe83a8c9dea1985025273c46423d31a0baafdc6361daffd54ab57cf1e0a55a7c8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1534846907;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2090357349;

        if (fTestNet)
        {
            block.nTime    = 1534846873;
            block.nNonce   = 386144368;
        }
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Bitwave: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitwaveMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitwaveMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitwaveMiner(CWallet *pwallet)
{
    printf("BitwaveMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitwave-miner");
    ...
    try { loop {
        ...
        printf("Running BitwaveMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitwaveMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitwaveMiner, pwallet));
}
